...
def neighbor_list(edges, current, CLOSED):
    ...
    for i in edges[:, 0:2]:
        if (current == i[1])and(not(i[0] in CLOSED))and (not(
                                                        current in nbr_list)):
            nbr_list.append(i[0])
    ...
    while(OPEN):
        ...
        for nbr in neighbor_list(edges, current, CLOSED):
            ...
            if(tentative_past_cost < past_cost[nbr]):
                ...
                # sort estimate total cost dictionary(item from low to
                # high value)
                sort_est_total_cost = dict(
                    sorted(est_total_cost.items(), key=lambda x: x[1]))
